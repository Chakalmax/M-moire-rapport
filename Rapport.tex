...
\label{Réparation}
	...
	\DontPrintSemicolon
  \caption{Data\_repairing($\Sigma$,$I$,$\C$):}
  \LinesNumbered
    \SetKwInOut{Input}{Entrée}
    \SetKwInOut{Output}{Sortie}

   % \underline{$\theta$-TolerantRepair} $(\mathbb{D},\Sigma,I)$\;
    \Input{Un ensemble de DC $\Sigma$, une relation $I$ à réparer et l'ensemble $\C$ des éléments à réparer.}
    \Output{Une réparation de $I$}
    Initialiser $f(I) = I$\;
    Initialiser $rc(\C,\Sigma)$\;
    \Pour{ chaque $t \in C$}
    {
	Résoudre $rc(\{t\},\Sigma)$ par un solveur  
	\Si{$rc(\{t\},\Sigma)$ peut être résolu}
	{
	$f(t)$ = solution du solveur	\;
	} {
	$t.A$ = l'élément apparaissant le plus de fois dans $rc(\{t\},\Sigma)$.
	$f(t.A)$ = $fv$\;
	}
    }
    \Return $f(I)$
...
\chapter{Variation de contraintes}
...
\label{variations}
  ...
  \LinesNumbered
    ...
    \Input{Une DC $\varphi$ dont on veut la variation}
    \Output{L'ensemble de toutes les variations de $\varphi$ respectant les conditions énoncés précédemment}
    Prendre un attribut $A$ quelconque de $S$.\;
    $variations$ = get\_recursive\_variations($A$,$\emptyset$,$\varphi$)\;
    \Pour{chaque $var$ dans $variations$}{
    \Si{$var$ est trivial \textbf{ou} $var$ n'est pas satisfiable \textbf{ou} $var$ n'est pas plus faible que $\varphi$}{
    Supprimer $var$ de $variations$\;
    }
    }
    \Return $variations$
\end{algorithm}\DecMargin{1em}
...
\label{variations_rec}
  ...
  \LinesNumbered
   ...
   % \underline{$\theta$-TolerantRepair} $(\mathbb{D},\Sigma,I)$\;
    \Input{Un attribut $A$, $att\_pris$ l'ensemble des attributs déjà utilisé dans les récursions précédentes, une DC $\varphi$ dont on veut la variation}
    \Output{Toutes les variations de $\varphi$ en modifiant les prédicats contenant $A$}
    Initialiser $\Sigma$ vide\;
    \Si{$|att\_pris| < |S|$ }
    {
    	\Pour{chaque prédicat $pred$ plus faible que $(A,\varphi(A)$}
    	{
   ...
   % \underline{$\theta$-TolerantRepair} $(\mathbb{D},\Sigma,I)$\;
	...
	\Pour{chaque variante de contrainte $\Sigma_i \in \mathbb{D}$}{
    	\Si{$\delta_l (\Sigma_i,I) \leq \delta_{min}$}
    		{
    		$I_i$ = DATAREPAIR($\Sigma_i,I,\mathbb{V}(G_i)$)\;
