...
\begin{table}[H]
	...
	\hline
	    & Nom & Anniversaire & Num\_Tel & Année & Revenu & Taxe\\
	\hline
...
\alinea Databases should be filled by values that respect some constraints related to the database. It would be a problem if it was possible to add non-logical value on some columns of a table. To avoid this kind of problems, we can add rules on a database. These rules are called \emph{constraints} and works on this way: if the entry row $t$ respects some conditions, we can accept the value. Otherwise $t$ is not correct and something is wrong with the value of this entry row.\\
...
$ NISS \rightarrow Nom, Prénom, Nai_Date, Dec_Date, Etat_Civil, Ville, Code_Post, Salaire, Taxe, Enfant$
...
Malheureusement les dépendances fonctionnelles sont limitées en terme de puissance, il existe de nombreuses contraintes que nous ne pouvons pas exprimer avec une DF. Par exemple, si nous souhaitons exprimer le fait que \emph{'Une personne ne peut être mort avant sa propre mort'}, nous avons besoin de comparer la $Nai\_Date$ et la $Dec\_Date$ de la personne et de s'assurer que la date de décès ne soit pas avant la date de naissance. Les dépendances fonctionnelles ne permettent pas d'utiliser des opérateurs de comparaison, il est donc nécessaire d'exprimer les contraintes d'une autre façon. Pour ce faire nous allons introduire un nouveau type de contrainte qui répondra bien à nos besoins: les \emph{denial constraints}.



%Nous pouvons par exemple exprimer cela avec la logique du premier ordre. Dans le cas \emph{' Une personne ne peut être mort avant sa propre mort'} nous aurions par exemple: 
%
%$$ \forall t : R(t) \rightarrow \neg[t(decease\_date)\leq t(birth\_date)]$$ or
%$$ \forall t : R(t) \rightarrow t(decease\_date) > t(birth\_date)$$
% 
%Nous pouvons également exprimer 
We can express previous constraints in first-order logic:

\begin{enumerate}
\item \emph{A NISS identify a person.}: 
%$\forall t_\alpha,t_\beta \in R$ $\neg(t_\alpha.NISS = t_\beta.NISS)$
$ \forall s \forall t : (R(S)\wedge R(t) \rightarrow s(Niss) = t(NISS) )$
\item \emph{Two persons with the same post code live in the same district.}: $ \forall s \forall t ( R(s)\wedge R(t) \wedge s(Post\_code) = t(Post\_code) \rightarrow s(District)= t(District) )$ %$\forall t_\alpha,t_\beta \in R$ $\neg(t_\alpha.post\_code = t_\beta.post\_code \wedge t_\alpha.district \neq t_\beta.district) $
\item \emph{If someone died the march 18$^{th}$ 1962 , his civil status should be equal to decease.}: $\forall t (R(t) \rightarrow \neg(t.Decease\_date = $'18-05-1962'$ \wedge t.Civil\_state = deceased)$.%$\forall t_\alpha \in R$ $\neg(t_\alpha.decease\_date = $'18-05-1962'$ \wedge t_\alpha.civil\_state = decease)$.
\end{enumerate}
...
Consider a relation scheme $R$ with $S$ a finite set of \emph{attribute}. A \textbf{denial constraint (DC)} over $S$ is a partial mapping from $S$ to the powerset $OP$ of $\{ <,=,> \}$. We will use the Greek letter $\varphi$ to denote a DC.

\end{mydef}

By definition a powerset of a set $S$ denoted $\mathcal{P}(S)$ is the set of all subset of $S$. It includes $S$ itself and also the empty set. It exists different syntactic shorthands  for elements of $OP$ as you can see in table \ref{operatorTable}.
We had to introduce 2 new operators $\top$ and $\bot$ defined as follows: For every $d_1,d_2 \in \mathbf{dom}$, $d_1\bot d_2$ is false and $d_1\top d_2$ is true. We will use the greek letter $\phi$ to denote operator.

\begin{table}
	\begin{center}
	\begin{tabular}{|c|c|}\hline
	Element & Abbreviation\\\hline\hline
	$\emptyset$ & $\bot$\\\hline
	$\{<\}$     & $<$\\\hline
	$\{=\}$     & $=$\\\hline
	$\{>\}$     & $>$\\\hline 
	...
	\end{tabular}
	\end{center}
	\caption{Element of OP, powerset of $\{ <,=,>\}$ \label{operatorTable}}
\end{table}
...
\begin{myexample}
Let $S= \{Name,Birtday,CP,Year,Income,Tax \}$. We can have the following denial constraints over S:
$\varphi_1 = \{(Name,=),(Birthday,=),(CP, \neq)\}$ and $\varphi_2 :\{(Income,>),(Tax,\neq)\}$
\end{myexample}

%TODO rewrite this part (copy-pasta!)
\begin{mydef}
Let $(\dom,\leq)$ be a totally ordered domain containing at least two distinct elements.
A {\em tuple over $S$\/} is a total mapping from $S$ to $\dom$.
A  {\em relation over $S$\/} is a finite set of tuples over $S$.
Let $\varphi$ be a denial constraint over $S$.
We say that a relation $I$ over $S$ satisfies $\varphi$,
denoted $I\models\varphi$,
if there exist no two tuples $s,t\in I$ such that for every attribute $A$ in the domain of $\varphi$, we have $s(A)\ \theta\ t(A)$ where $\theta=\varphi(A)$. We say that $\varphi$ is a \emph{valid} DC if $I\models\varphi$. 
\end{mydef}

\begin{myexample}
$\{(\ic,>)$, $(\ta,\leq)\}$ is satisfied by a relation $I$ if there exist no two tuples $s,t\in I$ such that $s(\ic)>t(\ic)$ and $s(\ta)\leq t(\ta)$.
In other words, $\{(\ic,>)$, $(\ta,\leq)\}$ is satisfied by $I$ if for all $s,t\in I$, if $s(\ic)>t(\ic)$, then $s(\ta)>t(\ta)$.
\end{myexample}

If we want to translate $\varphi_1$ into first-order logic it will be:
\begin{displaymath}
  \begin{split}
    \varphi : \forall s,t : R(t) \wedge R(s) \rightarrow &(s.Name = t(Name) \wedge s(CP) = t(CP) \wedge \\
    & s(Birthday) = t(Birthday))
  \end{split}
\end{displaymath}

In the main article \cite{main} they defined the DC as an expression of the the form $\forall s,t \in R \neg(P_1 \wedge P_2 \wedge ... \wedge P_m)$ where $P_i$ is in the from $s(A) \phi t(A)$ or $s(A) \phi c$ or $t(a) \phi c$  and $\phi$ in one of the element of $OP$. This expression is satisfied if if all the predicates are not true at the same time. In others words, this constraints is satisfied if at least one of the $P_i$ is false.  
...
\begin{displaymath}
  \begin{split}
\varphi : \forall s,t : R(t) \wedge R(s) \rightarrow &(s.Name = t(Name) \wedge s(CP) = t(CP) \wedge \\
    & s(Birthday) = t(Birthday)\wedge s(Year) = t(Year)
    \end{split}
\end{displaymath}
...
A DC can be useless and always true. Such DCs should not be present because they never identify any violation. In that case we say that the denial constraint is \emph{trivial}. We'll define a trivial DC as:

\begin{mydef}
	We say a denial constraint $\varphi$ is \textbf{trivial} if for any instance $I$  of $R$ we have $I \models \varphi$
\end{mydef}
...
\begin{myprop}
\label{trivialprop}
 $\forall P_i, P_j$ if $\overline{P_i} \in Imp(P_j)$, then $\neg(P_i \wedge P_j)$ is a trivial DC.
\end{myprop}
